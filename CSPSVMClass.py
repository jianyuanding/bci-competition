...
class CSPSVMClass:
    ...
    def recognize(self, data, personID):
        ...
        pro = [pro1, pro2, pro3]
        result = pro.index(min(pro)) + 200
        return result

    def recognize1(self, data, personID):
        mod_max_path = rootdir + '/model'
        mod_max_list = os.listdir(mod_max_path)
        modPath = os.path.join(mod_max_path, mod_max_list[personID - 1])
        mod = joblib.load(modPath)

        test = np.reshape(data, (1, data.shape[0] * data.shape[1]))
        predict = mod.predict(test)
        print(predict[0])
        return predict[0]
