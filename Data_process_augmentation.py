...
# S01 (0.5,40); S02 (8, 30); S03 (8, 30); S04 (8, 30); S05 (8, 30)

def band_Filter(rawdata):
    order, low_cut, high_cut, fs = 5, 1, 40, 250
...
def train_data_process(train_data_path):
    ...
    # 获取训练集和验证集，训练集是前70个样本及其扩展，验证样本是后20个样本
    train_data = Train_data[:420]
    validation_data = Train_data[420:540:6]

    label_train = np.array(classes_train, dtype=object).astype(int)
    train_label = label_train[0:70]
    validation_label = label_train[70:90]
    ...
    # label_train_copy = label_train.reshape(-1)-201
    # label_train_copy = np.tile(label_train.reshape(-1, 1), (1, 6)).reshape(-1) - 201
    # Train_label = np.eye(3)[label_train_copy]  # 转为one-hot形式
    label_train_copy = np.tile(train_label.reshape(-1, 1), (1, 6)).reshape(-1) - 201
    train_label = np.eye(3)[label_train_copy]  # 转为one-hot形式

    label_validation_copy = validation_label.reshape(-1) - 201
    validation_label = np.eye(3)[label_validation_copy]  # 转为one-hot形式
    ...
    Train_data = np.stack((train_data, train_data[:,::-1,:])).reshape(840, 20, 1000)
    Train_label = np.stack((train_label, train_label)).reshape(840, 3)

    # 打乱样本顺序
    shuffle_idx = np.random.permutation(Train_label.shape[0])
    train_data_shuffle, train_label_shuffle = Train_data[shuffle_idx], Train_label[shuffle_idx]

    shuffle_idx_val = np.random.permutation(validation_label.shape[0])
    validation_data_shuffle, validation_label_shuffle = validation_data[shuffle_idx_val], validation_label[
        shuffle_idx_val]

    return train_data_shuffle, train_label_shuffle, validation_data_shuffle, validation_label_shuffle
